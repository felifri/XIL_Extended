            ...
            for epoch in range(1, epochs+1):
                ...
                for batch, data in enumerate(dataloader):
                    ...
                    if epoch > disable_xil_loss_first_n_epochs:
                        ...
                        if isinstance(self.loss, RRRLoss):
                        ...
                        elif isinstance(self.loss, MixLoss2_ext):
                            X, y, expl_p, expl_r = data[0].to(self.device), data[1].to(self.device), data[2].to(self.device), data[3].to(self.device)
                            X.requires_grad_()
                            output = self.model(X)
                            expl_p = expl_p.float()
                            ...
                            expl_r = expl_r.float()
                            loss, ra_loss_c, rr_loss_c = self.loss(self.model, X, y, expl_p, expl_r, output, self.device)
                        elif isinstance(self.loss, MixLoss7_ext):
                            X, y, expl_p, expl_r = data[0].to(self.device), data[1].to(self.device), data[2].to(self.device), data[3].to(self.device)
                            X.requires_grad_()
                            output = self.model(X)
                            expl_p = expl_p.float()
                            ...
                            expl_r = expl_r.float()
                            loss, ra_loss_c, rr_loss_c = self.loss(self.model, X, y, expl_p, expl_r, output, self.device)
                        elif isinstance(self.loss, MixLoss9_ext):
                            X, y, expl_p, expl_r = data[0].to(self.device), data[1].to(self.device), data[2].to(self.device), data[3].to(self.device)
                            X.requires_grad_()
                            output = self.model(X)
                            expl_p = expl_p.float()
                    ...
                    if isinstance(self.loss, (RRRLoss, HINTLoss, CDEPLoss, RBRLoss, RRRGradCamLoss, HINTLoss_IG, MixLoss1, MixLoss2, MixLoss2_ext,\
                                              MixLoss3, MixLoss4, MixLoss5, MixLoss6, MixLoss7, MixLoss7_ext, MixLoss8, MixLoss8_ext, MixLoss9, MixLoss9_ext)) \
